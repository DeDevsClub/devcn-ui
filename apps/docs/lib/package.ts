...
export const getPackage = async (packageName: string) => {
  ...
  const devcnDependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) => dep.startsWith('@repo') && dep !== '@repo/shadcn-ui'
  );

  const dependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) =>
      !['react', 'react-dom', '@repo/shadcn-ui', ...devcnDependencies].includes(
        dep
      )
  );
  ...
  for (const dep of devcnDependencies) {
