...
The CLI will download the component's code and integrate it into your project's directory (usually under your components folder). By default, Design Registry components are added to the `@/components/ui/devcn/` directory (or whatever folder you've configured in your shadcn components settings).

For instance, the command above will place the AI Conversation component files in `@/components/ui/devcn/ai-conversation/`. The CLI also ensures that any required dependencies (for example, specific headless libraries a component might need) are installed automatically.
